@Student_Reschedule_class 
Feature: The student Reschedule the teacher class

Scenario Outline: The student Reschedule the class
    ...
    And  Click the reschedule button
    When Student view the time slot and choose the new date and time slot <date2> and <Time>
    And Studnet pick the new time slot then reschedule request send it to the teacher
    Then Finally the Student logout the whaot student user account
    
    Examples: 
      | phonenumber | OTP1 | OTP2 | OTP3 | OTP4 | date |date2|Time|
      |  8754986845		 |    1 |    2 |    3 |    4 | "18" |"19"| "9.00 am - 9.30 am"|
